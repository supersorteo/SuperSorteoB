...
public class RifaController {
    ...
    @PostMapping("/pago")
    public ResponseEntity<?> generarPago(@RequestBody Map<String, Object> payload) {
        try {
            ...
            Map<String, Object> preferencia = rifaService.crearPreferenciaPago(rifaId, amount, usuarioId);
            return ResponseEntity.ok(preferencia);
        } catch (ResourceNotFoundException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Rifa no encontrada: " + e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error al generar preferencia: " + e.getMessage());
        }
    }
