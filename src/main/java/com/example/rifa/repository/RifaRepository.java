...
public interface RifaRepository extends JpaRepository<Rifa, Long> {
    ...
    List<Rifa> findByExecutedTrue();

    @Query("SELECT COUNT(r) FROM Rifa r WHERE r.usuario.id = :usuarioId AND r.isActive = true")
    long countActiveRifasByUsuarioId(@Param("usuarioId") Integer usuarioId);

    @Query("SELECT COUNT(r) FROM Rifa r WHERE r.usuario.id = :usuarioId AND r.isActive = false AND r.winningNumber IS NULL")
    long countRifasSinGanadorByUsuarioId(@Param("usuarioId") Integer usuarioId);

    @Query("SELECT COUNT(r) FROM Rifa r WHERE r.usuario = :usuario AND r.isActive = true")
    long countByUsuarioAndIsActive(@Param("usuario") Usuario usuario);

    @Query("SELECT COUNT(r) FROM Rifa r WHERE r.usuario = :usuario AND r.codigoVipUsado = :codigoVip")
    long countByUsuarioAndCodigoVipUsado(@Param("usuario") Usuario usuario, @Param("codigoVip") String codigoVip);
